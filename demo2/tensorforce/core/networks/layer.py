...
class Expansion(Layer):
    """
    expand dims reshaping the input.
    """

    def __init__(self, scope='expansion', summary_labels=()):
        super(Expansion, self).__init__(scope=scope, summary_labels=summary_labels)

    def tf_apply(self, x, update):
        expanded_tensor=tf.expand_dims(x, -1)
        #print('expanded_tensor',np.shape(expanded_tensor))
